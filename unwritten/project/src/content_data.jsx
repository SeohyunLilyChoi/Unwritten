// Content page — mock data for the 콘텐츠 tab.
// Sections: 투표 (poll) / 아티클 (article) / 콘테스트 (contest) / 오늘의 단어 (word of day) / 팁·카드뉴스 (tip)
// 각 타입별 악센트 컬러로 시각적 구분.

const CONTENT_TYPES = {
  poll:    { key:'poll',    label:'투표',    kicker:'POLL',    color:'#3B5BFF', soft:'#EEF1FF', ink:'#1E3AD1', dot:'●' },
  article: { key:'article', label:'아티클',  kicker:'ARTICLE', color:'#D97757', soft:'#FBEFE8', ink:'#8E4322', dot:'■' },
  contest: { key:'contest', label:'콘테스트', kicker:'CONTEST', color:'#7A5BFF', soft:'#F1EDFF', ink:'#4B32B8', dot:'◆' },
  word:    { key:'word',    label:'오늘의 단어', kicker:'WORD',  color:'#16A34A', soft:'#E7F6EC', ink:'#0E6A2E', dot:'▲' },
  tip:     { key:'tip',     label:'팁',      kicker:'TIP',     color:'#0EA5E9', soft:'#E4F4FC', ink:'#0A6A95', dot:'✦' },
};

// Job/year chips for content filter (topic-level)
...
// Featured/hero — rotating picks for the top strip
const CONTENT_FEATURED = [
  { type:'poll',    topic:'time',   title:'선배님들, 출근 시간 보통 몇 분 전에 도착하시나요?', meta:'2,147명 참여 중', tag:'#출근시간' },
  { type:'article', topic:'comm',   title:'요즘 팀장들이 팀원과 소통하는 진짜 방법', meta:'5분 읽기 · 편집팀', tag:'#1on1' },
  { type:'contest', topic:'culture',title:'우리 회사 이상한 문화 공모전', meta:'D-4 · 상금 100만원', tag:'#공모' },
];
...
const CONTENT_POLLS = [
  {
    ...
    tag:'#출근시간', nParticipants: 2147, audience:'#신입 사원', daysLeft: 3,
    options: [
  ...
  },
  {
    ...
    tag:'#퇴근시간', nParticipants: 1284, audience:'#전체', daysLeft: 5,
    options: [
      { label:'A. 슬쩍 짐을 싼다', pct: 54 },
      { label:'B. 눈치 보며 기다린다', pct: 46 },
    ],
  },
  {
    id:'poll-3', type:'poll', topic:'comm',
    title:'회식 자리, 1차 끝나고 어떻게 하세요?',
    tag:'#회식', nParticipants: 892, audience:'#신입 사원', daysLeft: 2,
    options: [
      { label:'무조건 2차까지',     pct: 18 },
      { label:'분위기 봐서 결정',    pct: 47 },
      { label:'1차에서 빠지기',     pct: 35 },
    ],
  },
];
...
const CONTENT_WORDS = [
  ...
  {
    id:'word-2', type:'word', topic:'time',
    word:'TF', pron:'[티에프]',
    tag:'조직 용어',
    def:'Task Force. 특정 프로젝트를 위해 한시적으로 구성되는 팀. 끝나면 자동 해산.',
    example:'"이번에 신제품 TF 꾸려지는데 같이 하실래요?" — 대부분 업무 추가로 돌아옵니다.',
  },
];
...
window.CONTENT_TOPICS = CONTENT_TOPICS;
window.CONTENT_FEATURED = CONTENT_FEATURED;
window.CONTENT_POLLS = CONTENT_POLLS;
window.CONTENT_ARTICLES = CONTENT_ARTICLES;
window.CONTENT_CONTESTS = CONTENT_CONTESTS;
window.CONTENT_WORDS = CONTENT_WORDS;
window.CONTENT_TIPS = CONTENT_TIPS;
